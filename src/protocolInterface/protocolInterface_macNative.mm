...
#if !__has_feature(objc_arc)
			// Retain the semaphore for as long as we hold a count on it (until the matching signal in the completionHandler), so deinitEntity cannot dispose it while in use (which is a fatal error in libdispatch). If the completionHandler is never called, the semaphore leaks instead of crashing the process.
			dispatch_retain(limiter);
#endif
			// Take a semaphore count to limit the inflight commands
			dispatch_semaphore_wait(limiter, DISPATCH_TIME_FOREVER);

			[self startAsyncOperation];
			auto const sent = [self.interface.aecp sendCommand:message
																			 ...
																			 completionHandler:^(NSError* error, AVB17221AECPMessage* message) {
																				 {
																					 ...
																					 auto const lg = std::lock_guard{ _lock };
																					 // The resultHandler is consumed under the lock, since it can also be consumed by the sending code if the send method failed
																					 if (!resultHandler)
																					 {
...
#if !__has_feature(objc_arc)
																				 dispatch_release(limiter);
#endif
																			 }];
			// The framework is not guaranteed to call the completionHandler when it fails to send the message (e.g. when the interface is going down), so we have to cleanup ourselves in that case
			if (!sent)
			{
				// Lock Self before calling a handler, and consume the resultHandler under the lock in case the completionHandler is called anyway
				auto const lg = std::lock_guard{ _lock };
				if (resultHandler)
				{
					LOG_PROTOCOL_INTERFACE_WARN(_protocolInterface->getMacAddress(), macAddr, "Failed to send AECP command, macOS framework refused the message");
					la::avdecc::utils::invokeProtectedHandler(resultHandler, nullptr, la::avdecc::protocol::ProtocolInterface::Error::TransportError);
					resultHandler = {};
					[self stopAsyncOperation];
					// Release the semaphore count we took, and the retain that goes with it
					dispatch_semaphore_signal(limiter);
#if !__has_feature(objc_arc)
					dispatch_release(limiter);
#endif
...
- (la::avdecc::protocol::ProtocolInterface::Error)sendAcmpCommand:(la::avdecc::protocol::Acmpdu::UniquePointer&&)acmpdu handler:(la::avdecc::protocol::ProtocolInterface::AcmpCommandResultHandler const&)onResult {
	...
	[self startAsyncOperation];
	auto const sent = [self.interface.acmp sendACMPCommandMessage:message
	...
	if (!sent)
	{
		...
		if (resultHandler)
		{
			LOG_PROTOCOL_INTERFACE_WARN(_protocolInterface->getMacAddress(), acmp.getDestAddress(), "Failed to send ACMP command, macOS framework refused the message");
			la::avdecc::utils::invokeProtectedHandler(resultHandler, nullptr, la::avdecc::protocol::ProtocolInterface::Error::TransportError);
			resultHandler = {};
			[self stopAsyncOperation];
		}
