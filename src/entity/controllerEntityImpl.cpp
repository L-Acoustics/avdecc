...
/* ************************************************************************** */
void ControllerEntityImpl::sendAemAecpCommand(UniqueIdentifier const targetEntityID, protocol::AemCommandType const commandType, void const* const payload, size_t const payloadLength, OnAemAECPErrorCallback const& onErrorCallback, AnswerCallback const& answerCallback) const noexcept
{
	try
	{
...
void ControllerEntityImpl::sendAaAecpCommand(UniqueIdentifier const targetEntityID, addressAccess::Tlvs const& tlvs, OnAaAECPErrorCallback const& onErrorCallback, AnswerCallback const& answerCallback) const noexcept
{
	try
	{
		auto* pi = const_cast<ControllerEntityImpl*>(this)->getProtocolInterface();
		...
		if (!networkInterface::isMacAddressValid(targetMacAddress))
		{
			invokeProtectedHandler(onErrorCallback, AaCommandStatus::UnknownEntity);
			return;
		}
	...
	{
		invokeProtectedHandler(onErrorCallback, AaCommandStatus::ProtocolError);
	}
	catch (...)
	{
...
void ControllerEntityImpl::processAemAecpResponse(protocol::Aecpdu const* const response, OnAemAECPErrorCallback const& onErrorCallback, AnswerCallback const& answerCallback) const noexcept
{
	...
	// Deserialize payload
				...
				auto* delegate = controller->getControllerDelegate();
				if ((flags & protocol::AemAcquireEntityFlags::Release) == protocol::AemAcquireEntityFlags::Release)
				{
					...
					{
						invokeProtectedMethod(&controller::Delegate::onEntityReleased, delegate, controller, targetID, ownerID, descriptorType, descriptorIndex);
					}
				}
				else
				{
					...
					if (aem.getUnsolicited() && delegate && !!status)
					{
						invokeProtectedMethod(&controller::Delegate::onEntityAcquired, delegate, controller, targetID, ownerID, descriptorType, descriptorIndex);
					}
				}
			}
		},
	...
	// Deserialize payload
		...
		//entity::model::DescriptorIndex const descriptorIndex = std::get<3>(result);
				...
				if ((flags & protocol::AemLockEntityFlags::Unlock) == protocol::AemLockEntityFlags::Unlock)
				{
					...
					/*if (aem.getUnsolicited() && delegate && !!status)
					{
						invokeProtectedMethod(&controller::Delegate::onEntityUnlocked, delegate, controller, targetID, lockedID);
					}*/
				}
				else
				{
					...
					/*if (aem.getUnsolicited() && delegate && !!status)
					{
						invokeProtectedMethod(&controller::Delegate::onEntityLocked, delegate, controller, targetID, lockedID);
					}*/
				}
			}
		},
	...
	// Deserialize payload
				...
				if (aem.getUnsolicited() && delegate && !!status)
				{
					invokeProtectedMethod(&controller::Delegate::onConfigurationChanged, delegate, controller, targetID, configurationIndex);
				}
			}
		},
		// Get Configuration
	...
	// Deserialize payload
				...
				if (descriptorType == model::DescriptorType::StreamInput)
				{
					...
					if (aem.getUnsolicited() && delegate && !!status)
					{
						invokeProtectedMethod(&controller::Delegate::onStreamInputFormatChanged, delegate, controller, targetID, descriptorIndex, streamFormat);
					}
				}
				else if (descriptorType == model::DescriptorType::StreamOutput)
				{
					...
					if (aem.getUnsolicited() && delegate && !!status)
					{
						invokeProtectedMethod(&controller::Delegate::onStreamOutputFormatChanged, delegate, controller, targetID, descriptorIndex, streamFormat);
					}
				}
				else
	...
	// Deserialize payload
				...
				if (descriptorType == model::DescriptorType::StreamInput)
				{
					...
					if (aem.getUnsolicited() && delegate && !!status)
					{
						invokeProtectedMethod(&controller::Delegate::onStreamInputInfoChanged, delegate, controller, targetID, descriptorIndex, streamInfo);
					}
				}
				else if (descriptorType == model::DescriptorType::StreamOutput)
				{
					...
					if (aem.getUnsolicited() && delegate && !!status)
					{
						invokeProtectedMethod(&controller::Delegate::onStreamOutputInfoChanged, delegate, controller, targetID, descriptorIndex, streamInfo);
					}
				}
				else
					throw InvalidDescriptorTypeException();
					...
					case model::DescriptorType::Entity:
					{
						...
						switch (nameIndex)
						{
							case 0: // entity_name
								...
								if (aem.getUnsolicited() && delegate && !!status)
								{
									invokeProtectedMethod(&controller::Delegate::onEntityNameChanged, delegate, controller, targetID, name);
								}
								break;
							case 1: // group_name
								...
								if (aem.getUnsolicited() && delegate && !!status)
								{
									invokeProtectedMethod(&controller::Delegate::onEntityGroupNameChanged, delegate, controller, targetID, name);
								}
								break;
							default:
					...
					case model::DescriptorType::Configuration:
					{
						...
						switch (nameIndex)
						{
							case 0: // object_name
								...
								if (aem.getUnsolicited() && delegate && !!status)
								{
									invokeProtectedMethod(&controller::Delegate::onConfigurationNameChanged, delegate, controller, targetID, descriptorIndex, name);
								}
								break;
							default:
					...
					case model::DescriptorType::AudioUnit:
					{
						switch (nameIndex)
						{
							case 0: // object_name
								...
								if (aem.getUnsolicited() && delegate && !!status)
								{
									invokeProtectedMethod(&controller::Delegate::onAudioUnitNameChanged, delegate, controller, targetID, configurationIndex, descriptorIndex, name);
								}
								break;
							default:
					...
					case model::DescriptorType::StreamInput:
					{
						switch (nameIndex)
						{
							case 0: // stream_name
								...
								if (aem.getUnsolicited() && delegate && !!status)
								{
									invokeProtectedMethod(&controller::Delegate::onStreamInputNameChanged, delegate, controller, targetID, configurationIndex, descriptorIndex, name);
								}
								break;
							default:
					...
					case model::DescriptorType::StreamOutput:
					{
						switch (nameIndex)
						{
							case 0: // stream_name
								...
								if (aem.getUnsolicited() && delegate && !!status)
								{
									invokeProtectedMethod(&controller::Delegate::onStreamOutputNameChanged, delegate, controller, targetID, configurationIndex, descriptorIndex, name);
								}
								break;
							default:
					...
					case model::DescriptorType::AvbInterface:
					{
						switch (nameIndex)
						{
							case 0: // object_name
								...
								if (aem.getUnsolicited() && delegate && !!status)
								{
									invokeProtectedMethod(&controller::Delegate::onAvbInterfaceNameChanged, delegate, controller, targetID, configurationIndex, descriptorIndex, name);
								}
								break;
							default:
					...
					case model::DescriptorType::ClockSource:
					{
						switch (nameIndex)
						{
							case 0: // object_name
								...
								if (aem.getUnsolicited() && delegate && !!status)
								{
									invokeProtectedMethod(&controller::Delegate::onClockSourceNameChanged, delegate, controller, targetID, configurationIndex, descriptorIndex, name);
								}
								break;
							default:
					...
					case model::DescriptorType::MemoryObject:
					{
						switch (nameIndex)
						{
							case 0: // object_name
								...
								if (aem.getUnsolicited() && delegate && !!status)
								{
									invokeProtectedMethod(&controller::Delegate::onMemoryObjectNameChanged, delegate, controller, targetID, configurationIndex, descriptorIndex, name);
								}
								break;
							default:
					...
					case model::DescriptorType::AudioCluster:
					{
						switch (nameIndex)
						{
							case 0: // object_name
								...
								if (aem.getUnsolicited() && delegate && !!status)
								{
									invokeProtectedMethod(&controller::Delegate::onAudioClusterNameChanged, delegate, controller, targetID, configurationIndex, descriptorIndex, name);
								}
								break;
							default:
					...
					case model::DescriptorType::ClockDomain:
					{
						switch (nameIndex)
						{
							case 0: // object_name
								...
								if (aem.getUnsolicited() && delegate && !!status)
								{
									invokeProtectedMethod(&controller::Delegate::onClockDomainNameChanged, delegate, controller, targetID, configurationIndex, descriptorIndex, name);
								}
								break;
							default:
	...
	// Deserialize payload
				...
				if (descriptorType == model::DescriptorType::AudioUnit)
				{
					...
					if (aem.getUnsolicited() && delegate && !!status)
					{
						invokeProtectedMethod(&controller::Delegate::onAudioUnitSamplingRateChanged, delegate, controller, targetID, descriptorIndex, samplingRate);
					}
				}
				else if (descriptorType == model::DescriptorType::VideoCluster)
				{
					...
					if (aem.getUnsolicited() && delegate && !!status)
					{
						invokeProtectedMethod(&controller::Delegate::onVideoClusterSamplingRateChanged, delegate, controller, targetID, descriptorIndex, samplingRate);
					}
				}
				else if (descriptorType == model::DescriptorType::SensorCluster)
				{
					...
					if (aem.getUnsolicited() && delegate && !!status)
					{
						invokeProtectedMethod(&controller::Delegate::onSensorClusterSamplingRateChanged, delegate, controller, targetID, descriptorIndex , samplingRate);
					}
				}
				else
					throw InvalidDescriptorTypeException();
	...
	// Deserialize payload
				...
				if (aem.getUnsolicited() && delegate && !!status)
				{
					invokeProtectedMethod(&controller::Delegate::onClockSourceChanged, delegate, controller, targetID, descriptorIndex, clockSourceIndex);
				}
			}
		},
		// Get Clock Source
	...
	// Deserialize payload
				...
				if (descriptorType == model::DescriptorType::StreamInput)
				{
					...
					if (aem.getUnsolicited() && delegate && !!status)
					{
						invokeProtectedMethod(&controller::Delegate::onStreamInputStarted, delegate, controller, targetID, descriptorIndex);
					}
				}
				else if (descriptorType == model::DescriptorType::StreamOutput)
				{
					...
					if (aem.getUnsolicited() && delegate && !!status)
					{
						invokeProtectedMethod(&controller::Delegate::onStreamOutputStarted, delegate, controller, targetID, descriptorIndex);
					}
				}
				else
					throw InvalidDescriptorTypeException();
	...
	// Deserialize payload
				...
				if (descriptorType == model::DescriptorType::StreamInput)
				{
					...
					if (aem.getUnsolicited() && delegate && !!status)
					{
						invokeProtectedMethod(&controller::Delegate::onStreamInputStopped, delegate, controller, targetID, descriptorIndex);
					}
				}
				else if (descriptorType == model::DescriptorType::StreamOutput)
				{
					...
					if (aem.getUnsolicited() && delegate && !!status)
					{
						invokeProtectedMethod(&controller::Delegate::onStreamOutputStopped, delegate, controller, targetID, descriptorIndex);
					}
				}
				else
					throw InvalidDescriptorTypeException();
	...
	// Deserialize payload
				...
				if (descriptorType == model::DescriptorType::AvbInterface)
				{
					...
					if (aem.getUnsolicited() && delegate && !!status)
					{
						invokeProtectedMethod(&controller::Delegate::onAvbInfoChanged, delegate, controller, targetID, descriptorIndex, avbInfo);
					}
				}
				else
					throw InvalidDescriptorTypeException();
	...
	// Deserialize payload
				...
				switch (descriptorType)
				{
					case model::DescriptorType::AvbInterface:
					{
						...
						if (aem.getUnsolicited() && delegate && !!status)
						{
							invokeProtectedMethod(&controller::Delegate::onAvbInterfaceCountersChanged, delegate, controller, targetID, descriptorIndex, flags, counters);
						}
						break;
					}
					case model::DescriptorType::ClockDomain:
					{
						...
						if (aem.getUnsolicited() && delegate && !!status)
						{
							invokeProtectedMethod(&controller::Delegate::onClockDomainCountersChanged, delegate, controller, targetID, descriptorIndex, flags, counters);
						}
						break;
					}
					case model::DescriptorType::StreamInput:
					{
						...
						if (aem.getUnsolicited() && delegate && !!status)
						{
							invokeProtectedMethod(&controller::Delegate::onStreamInputCountersChanged, delegate, controller, targetID, descriptorIndex, flags, counters);
						}
						break;
					}
					default:
	...
	// Deserialize payload
				...
				if (descriptorType == model::DescriptorType::StreamPortInput)
				{
					...
					if (aem.getUnsolicited() && delegate && !!status)
					{
						invokeProtectedMethod(&controller::Delegate::onStreamPortInputAudioMappingsChanged, delegate, controller, targetID, descriptorIndex, numberOfMaps, mapIndex, mappings);
					}
				}
				else if (descriptorType == model::DescriptorType::StreamPortOutput)
				{
					...
					if (aem.getUnsolicited() && delegate && !!status)
					{
						invokeProtectedMethod(&controller::Delegate::onStreamPortOutputAudioMappingsChanged, delegate, controller, targetID, descriptorIndex, numberOfMaps, mapIndex, mappings);
					}
				}
				else
					throw InvalidDescriptorTypeException();
...
#pragma message("TODO: Handle unsolicited notification (add handler, and handle it in controller code)")
	...
	// Deserialize payload
				...
				// Notify handlers
				answerCallback.invoke<AbortOperationHandler>(controller, targetID, status, descriptorType, descriptorIndex, operationID);
			}
		},
		// Operation Status
	...
	// Deserialize payload
				...
				invokeProtectedMethod(&controller::Delegate::onOperationStatus, delegate, controller, targetID, descriptorType, descriptorIndex, operationID, percentComplete);
			}
		},
	...
	// Deserialize payload
				...
				if (aem.getUnsolicited() && delegate && !!status)
				{
					invokeProtectedMethod(&controller::Delegate::onMemoryObjectLengthChanged, delegate, controller, targetID, configurationIndex, memoryObjectIndex, length);
				}
			}
		},
...
void ControllerEntityImpl::processAaAecpResponse(protocol::Aecpdu const* const response, OnAaAECPErrorCallback const& /*onErrorCallback*/, AnswerCallback const& answerCallback) const noexcept
{
	auto const& aa = static_cast<protocol::AaAecpdu const&>(*response);
	auto const status = static_cast<AaCommandStatus>(aa.getStatus().getValue()); // We have to convert protocol status to our extended status
	...
	static std::unordered_map<protocol::AcmpMessageType::value_type, std::function<void(ControllerEntityImpl const* const controller, ControllerEntity::ControlStatus const status, protocol::Acmpdu const& acmp, AnswerCallback const& answerCallback, bool const sniffed)>> s_Dispatch{
		...
		{ protocol::AcmpMessageType::ConnectTxResponse.getValue(),
			[](ControllerEntityImpl const* const controller, ControllerEntity::ControlStatus const status, protocol::Acmpdu const& acmp, AnswerCallback const& /*answerCallback*/, bool const sniffed)
			{
				...
				if (sniffed && delegate)
				{
					invokeProtectedMethod(&controller::Delegate::onListenerConnectResponseSniffed, delegate, controller, model::StreamIdentification{ talkerEntityID, talkerStreamIndex }, model::StreamIdentification{ listenerEntityID, listenerStreamIndex }, connectionCount, flags, status);
				}
			} },
		...
		{ protocol::AcmpMessageType::DisconnectTxResponse.getValue(),
			[](ControllerEntityImpl const* const controller, ControllerEntity::ControlStatus const status, protocol::Acmpdu const& acmp, AnswerCallback const& answerCallback, bool const sniffed)
			{
				...
				if (sniffed && delegate)
				{
					invokeProtectedMethod(&controller::Delegate::onListenerDisconnectResponseSniffed, delegate, controller, model::StreamIdentification{ talkerEntityID, talkerStreamIndex }, model::StreamIdentification{ listenerEntityID, listenerStreamIndex }, connectionCount, flags, status);
				}
			} },
		...
		{ protocol::AcmpMessageType::GetTxStateResponse.getValue(),
			[](ControllerEntityImpl const* const controller, ControllerEntity::ControlStatus const status, protocol::Acmpdu const& acmp, AnswerCallback const& answerCallback, bool const sniffed)
			{
				...
				if (sniffed && delegate)
				{
					invokeProtectedMethod(&controller::Delegate::onGetTalkerStreamStateResponseSniffed, delegate, controller, model::StreamIdentification{ talkerEntityID, talkerStreamIndex }, model::StreamIdentification{ listenerEntityID, listenerStreamIndex }, connectionCount, flags, status);
				}
			} },
		// Connect RX response
		{ protocol::AcmpMessageType::ConnectRxResponse.getValue(),
			[](ControllerEntityImpl const* const controller, ControllerEntity::ControlStatus const status, protocol::Acmpdu const& acmp, AnswerCallback const& answerCallback, bool const sniffed)
			{
				...
				if (sniffed && delegate)
				{
					invokeProtectedMethod(&controller::Delegate::onControllerConnectResponseSniffed, delegate, controller, model::StreamIdentification{ talkerEntityID, talkerStreamIndex }, model::StreamIdentification{ listenerEntityID, listenerStreamIndex }, connectionCount, flags, status);
				}
			} },
		...
		{ protocol::AcmpMessageType::DisconnectRxResponse.getValue(),
			[](ControllerEntityImpl const* const controller, ControllerEntity::ControlStatus const status, protocol::Acmpdu const& acmp, AnswerCallback const& answerCallback, bool const sniffed)
			{
				...
				if (sniffed && delegate)
				{
					invokeProtectedMethod(&controller::Delegate::onControllerDisconnectResponseSniffed, delegate, controller, model::StreamIdentification{ talkerEntityID, talkerStreamIndex }, model::StreamIdentification{ listenerEntityID, listenerStreamIndex }, connectionCount, flags, status);
				}
			} },
		...
		{ protocol::AcmpMessageType::GetRxStateResponse.getValue(),
			[](ControllerEntityImpl const* const controller, ControllerEntity::ControlStatus const status, protocol::Acmpdu const& acmp, AnswerCallback const& answerCallback, bool const sniffed)
			{
				...
				if (sniffed && delegate)
				{
					invokeProtectedMethod(&controller::Delegate::onGetListenerStreamStateResponseSniffed, delegate, controller, model::StreamIdentification{ talkerEntityID, talkerStreamIndex }, model::StreamIdentification{ listenerEntityID, listenerStreamIndex }, connectionCount, flags, status);
				}
			} },
...
void ControllerEntityImpl::acquireEntity(UniqueIdentifier const targetEntityID, bool const isPersistent, model::DescriptorType const descriptorType, model::DescriptorIndex const descriptorIndex, AcquireEntityHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, UniqueIdentifier::getNullUniqueIdentifier(), descriptorType, descriptorIndex);
...
void ControllerEntityImpl::lockEntity(UniqueIdentifier const targetEntityID, LockEntityHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, UniqueIdentifier::getNullUniqueIdentifier());
...
void ControllerEntityImpl::unlockEntity(UniqueIdentifier const targetEntityID, UnlockEntityHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1);
...
{
	auto errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1);
...
void ControllerEntityImpl::readEntityDescriptor(UniqueIdentifier const targetEntityID, EntityDescriptorHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, model::EntityDescriptor{});
...
void ControllerEntityImpl::readConfigurationDescriptor(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, ConfigurationDescriptorHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, model::ConfigurationDescriptor{});
...
void ControllerEntityImpl::readAudioUnitDescriptor(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::AudioUnitIndex const audioUnitIndex, AudioUnitDescriptorHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, audioUnitIndex, model::AudioUnitDescriptor{});
...
void ControllerEntityImpl::readStreamInputDescriptor(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::StreamIndex const streamIndex, StreamInputDescriptorHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, streamIndex, model::StreamDescriptor{});
...
void ControllerEntityImpl::readJackInputDescriptor(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::JackIndex const jackIndex, JackInputDescriptorHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, jackIndex, model::JackDescriptor{});
...
void ControllerEntityImpl::readAvbInterfaceDescriptor(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::AvbInterfaceIndex const avbInterfaceIndex, AvbInterfaceDescriptorHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, avbInterfaceIndex, model::AvbInterfaceDescriptor{});
...
void ControllerEntityImpl::readClockSourceDescriptor(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::ClockSourceIndex const clockSourceIndex, ClockSourceDescriptorHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, clockSourceIndex, model::ClockSourceDescriptor{});
...
void ControllerEntityImpl::readMemoryObjectDescriptor(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::MemoryObjectIndex const memoryObjectIndex, MemoryObjectDescriptorHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, memoryObjectIndex, model::MemoryObjectDescriptor{});
...
void ControllerEntityImpl::readLocaleDescriptor(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::LocaleIndex const localeIndex, LocaleDescriptorHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, localeIndex, model::LocaleDescriptor{});
...
void ControllerEntityImpl::readStringsDescriptor(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::StringsIndex const stringsIndex, StringsDescriptorHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, stringsIndex, model::StringsDescriptor{});
...
void ControllerEntityImpl::readStreamPortInputDescriptor(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::StreamPortIndex const streamPortIndex, StreamPortInputDescriptorHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, streamPortIndex, model::StreamPortDescriptor{});
...
void ControllerEntityImpl::readExternalPortInputDescriptor(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::ExternalPortIndex const externalPortIndex, ExternalPortInputDescriptorHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, externalPortIndex, model::ExternalPortDescriptor{});
...
void ControllerEntityImpl::readInternalPortInputDescriptor(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::InternalPortIndex const internalPortIndex, InternalPortInputDescriptorHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, internalPortIndex, model::InternalPortDescriptor{});
...
void ControllerEntityImpl::readAudioClusterDescriptor(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::ClusterIndex const clusterIndex, AudioClusterDescriptorHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, clusterIndex, model::AudioClusterDescriptor{});
...
void ControllerEntityImpl::readAudioMapDescriptor(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::MapIndex const mapIndex, AudioMapDescriptorHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, mapIndex, model::AudioMapDescriptor{});
...
void ControllerEntityImpl::readClockDomainDescriptor(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::ClockDomainIndex const clockDomainIndex, ClockDomainDescriptorHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, clockDomainIndex, model::ClockDomainDescriptor{});
...
void ControllerEntityImpl::setConfiguration(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, SetConfigurationHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex);
...
void ControllerEntityImpl::setStreamInputFormat(UniqueIdentifier const targetEntityID, model::StreamIndex const streamIndex, model::StreamFormat const streamFormat, SetStreamInputFormatHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, streamIndex, model::StreamFormat());
...
void ControllerEntityImpl::getStreamPortInputAudioMap(UniqueIdentifier const targetEntityID, model::StreamPortIndex const streamPortIndex, model::MapIndex const mapIndex, GetStreamPortInputAudioMapHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, streamPortIndex, model::MapIndex(0), mapIndex, s_emptyMappings);
...
void ControllerEntityImpl::addStreamPortInputAudioMappings(UniqueIdentifier const targetEntityID, model::StreamPortIndex const streamPortIndex, model::AudioMappings const& mappings, AddStreamPortInputAudioMappingsHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, streamPortIndex, s_emptyMappings);
...
void ControllerEntityImpl::getStreamInputInfo(UniqueIdentifier const targetEntityID, model::StreamIndex const streamIndex, GetStreamInputInfoHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, streamIndex, s_emptyStreamInfo);
...
void ControllerEntityImpl::getEntityName(UniqueIdentifier const targetEntityID, GetEntityNameHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, s_emptyAvdeccFixedString);
...
void ControllerEntityImpl::getConfigurationName(UniqueIdentifier const targetEntityID, entity::model::ConfigurationIndex const configurationIndex, GetConfigurationNameHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, s_emptyAvdeccFixedString);
...
void ControllerEntityImpl::setAudioUnitName(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::AudioUnitIndex const audioUnitIndex, model::AvdeccFixedString const& audioUnitName, SetAudioUnitNameHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, audioUnitIndex);
...
void ControllerEntityImpl::getAudioUnitName(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::StreamIndex const audioUnitIndex, GetAudioUnitNameHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, audioUnitIndex, s_emptyAvdeccFixedString);
...
void ControllerEntityImpl::setStreamInputName(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::StreamIndex const streamIndex, model::AvdeccFixedString const& streamInputName, SetStreamInputNameHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, streamIndex);
...
void ControllerEntityImpl::getStreamInputName(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::StreamIndex const streamIndex, GetStreamInputNameHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, streamIndex, s_emptyAvdeccFixedString);
...
void ControllerEntityImpl::setAvbInterfaceName(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::AvbInterfaceIndex const avbInterfaceIndex, model::AvdeccFixedString const& avbInterfaceName, SetAvbInterfaceNameHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, avbInterfaceIndex);
...
void ControllerEntityImpl::getAvbInterfaceName(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::StreamIndex const avbInterfaceIndex, GetAvbInterfaceNameHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, avbInterfaceIndex, s_emptyAvdeccFixedString);
...
void ControllerEntityImpl::setClockSourceName(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::ClockSourceIndex const clockSourceIndex, model::AvdeccFixedString const& clockSourceName, SetClockSourceNameHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, clockSourceIndex);
...
void ControllerEntityImpl::getClockSourceName(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::StreamIndex const clockSourceIndex, GetClockSourceNameHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, clockSourceIndex, s_emptyAvdeccFixedString);
...
void ControllerEntityImpl::setMemoryObjectName(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::MemoryObjectIndex const memoryObjectIndex, model::AvdeccFixedString const& memoryObjectName, SetMemoryObjectNameHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, memoryObjectIndex);
...
void ControllerEntityImpl::getMemoryObjectName(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::StreamIndex const memoryObjectIndex, GetMemoryObjectNameHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, memoryObjectIndex, s_emptyAvdeccFixedString);
...
void ControllerEntityImpl::setAudioClusterName(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::ClusterIndex const audioClusterIndex, model::AvdeccFixedString const& audioClusterName, SetAudioClusterNameHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, audioClusterIndex);
...
void ControllerEntityImpl::getAudioClusterName(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::StreamIndex const audioClusterIndex, GetAudioClusterNameHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, audioClusterIndex, s_emptyAvdeccFixedString);
...
void ControllerEntityImpl::setClockDomainName(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::ClockDomainIndex const clockDomainIndex, model::AvdeccFixedString const& clockDomainName, SetClockDomainNameHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, clockDomainIndex);
...
void ControllerEntityImpl::getClockDomainName(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::StreamIndex const clockDomainIndex, GetClockDomainNameHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, clockDomainIndex, s_emptyAvdeccFixedString);
...
void ControllerEntityImpl::setAudioUnitSamplingRate(UniqueIdentifier const targetEntityID, model::AudioUnitIndex const audioUnitIndex, model::SamplingRate const samplingRate, SetAudioUnitSamplingRateHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, audioUnitIndex, model::getNullSamplingRate());
...
void ControllerEntityImpl::setVideoClusterSamplingRate(UniqueIdentifier const targetEntityID, model::ClusterIndex const videoClusterIndex, model::SamplingRate const samplingRate, SetVideoClusterSamplingRateHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, videoClusterIndex, model::getNullSamplingRate());
...
void ControllerEntityImpl::setSensorClusterSamplingRate(UniqueIdentifier const targetEntityID, model::ClusterIndex const sensorClusterIndex, model::SamplingRate const samplingRate, SetSensorClusterSamplingRateHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, sensorClusterIndex, model::getNullSamplingRate());
...
void ControllerEntityImpl::setClockSource(UniqueIdentifier const targetEntityID, model::ClockDomainIndex const clockDomainIndex, model::ClockSourceIndex const clockSourceIndex, SetClockSourceHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, clockDomainIndex, model::ClockSourceIndex{ 0u });
...
void ControllerEntityImpl::startStreamInput(UniqueIdentifier const targetEntityID, model::StreamIndex const streamIndex, StartStreamInputHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, streamIndex);
...
void ControllerEntityImpl::getAvbInfo(UniqueIdentifier const targetEntityID, model::AvbInterfaceIndex const avbInterfaceIndex, GetAvbInfoHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, avbInterfaceIndex, s_emptyAvbInfo);
...
void ControllerEntityImpl::abortOperation(UniqueIdentifier const targetEntityID, model::DescriptorType const descriptorType, model::DescriptorIndex const descriptorIndex, model::OperationID const operationID, AbortOperationHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, descriptorType, descriptorIndex, operationID);
...
void ControllerEntityImpl::setMemoryObjectLength(UniqueIdentifier const targetEntityID, model::ConfigurationIndex const configurationIndex, model::MemoryObjectIndex const memoryObjectIndex, std::uint64_t const length, SetMemoryObjectLengthHandler const& handler) const noexcept
{
	try
	{
		...
		auto const errorCallback = ControllerEntityImpl::makeAemAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, configurationIndex, memoryObjectIndex, std::uint64_t{ 0u });
...
void ControllerEntityImpl::addressAccess(la::avdecc::UniqueIdentifier const targetEntityID, addressAccess::Tlvs const& tlvs, AddressAccessHandler const& handler) const noexcept
{
	try
	{
		auto const errorCallback = ControllerEntityImpl::makeAaAECPErrorHandler(handler, this, targetEntityID, std::placeholders::_1, addressAccess::Tlvs{});
...
controller::Delegate* ControllerEntityImpl::getControllerDelegate() const noexcept
{
	return _delegate;
}
...
{
	invokeProtectedMethod(&controller::Delegate::onTransportError, _delegate, this);
}
...
void ControllerEntityImpl::onRemoteEntityOnline(protocol::ProtocolInterface* const pi, Entity const& entity) noexcept
{
	auto const entityID = entity.getEntityID();
	{
		...
		// Store or replace entity
		{
			...
			AVDECC_ASSERT(_discoveredEntities.find(entityID) == _discoveredEntities.end(), "ControllerEntityImpl::onRemoteEntityOnline: Entity already online");
	...
	invokeProtectedMethod(&controller::Delegate::onEntityOnline, _delegate, this, entityID, entity);
}
...
void ControllerEntityImpl::onRemoteEntityOffline(protocol::ProtocolInterface* const pi, UniqueIdentifier const entityID) noexcept
{
	...
	invokeProtectedMethod(&controller::Delegate::onEntityOffline, _delegate, this, entityID);
}
...
void ControllerEntityImpl::onRemoteEntityUpdated(protocol::ProtocolInterface* const pi, Entity const& entity) noexcept
{
	auto const entityID = entity.getEntityID();
	{
		...
		{
#ifdef __cpp_lib_unordered_map_try_emplace
			AVDECC_ASSERT(_discoveredEntities.find(entityID) != _discoveredEntities.end(), "ControllerEntityImpl::onRemoteEntityUpdated: Entity offline");
			_discoveredEntities.insert_or_assign(entityID, entity);
#else // !__cpp_lib_unordered_map_try_emplace
			auto it = _discoveredEntities.find(entityID);
	...
	invokeProtectedMethod(&controller::Delegate::onEntityUpdate, _delegate, this, entityID, entity);
}
...
void ControllerEntityImpl::onAecpUnsolicitedResponse(protocol::ProtocolInterface* const /*pi*/, entity::LocalEntity const& /*entity*/, protocol::Aecpdu const& aecpdu) noexcept
{
	...
	if (aecpdu.getControllerEntityID() != getEntityID())
		return;
...
{
	// Ignore messages not for me
	if (aecpdu.getTargetEntityID() != getEntityID())
		return false;
